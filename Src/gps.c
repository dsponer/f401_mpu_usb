...
//    min = fmod((double) degMin, 100.0);
    min = (double) degMin / 100.0 - (int) ((double) degMin / 100.0);
    ...
    if ((HAL_GetTick() - GPS.LastTime > 50) && (GPS.rxIndex > 0)) {
        ...
        if (str != NULL) {
            ...
            sscanf(str, "$GPGGA,%2hhd%2hhd%2hhd.%3hd,%f,%c,%f,%c,%hhd,%hhd,%f,%f,%c,%hd,%s,*%2s\r\n",
                   ...
                   &GPS.GPGGA.Latitude, &GPS.GPGGA.NS_Indicator, &GPS.GPGGA.Longitude, &GPS.GPGGA.EW_Indicator,
                   &GPS.GPGGA.PositionFixIndicator, &GPS.GPGGA.SatellitesUsed, &GPS.GPGGA.HDOP, &GPS.GPGGA.MSL_Altitude,
                   &GPS.GPGGA.MSL_Units, &GPS.GPGGA.AgeofDiffCorr, GPS.GPGGA.DiffRefStationID, GPS.GPGGA.CheckSum);
